...
filename = 'vicon_no_arm_auto_cut_2016-01-21-12-01-46_2';
file = [filename,'.bag'];
filefolder = '/home/kelly/Desktop/master_thesis/complete_experiments'
...
robot_dist_to_tree = 0.5;

camera_to_robot_translation = [0.136465, 0.00016, -0.0855532];
camera_to_robot_rotation_rotm = [0.00301025  -0.287395   0.957807 ; -0.999993 0.00133665  0.0035439 ; -0.00229875  -0.957811  -0.287389]


disp('Set params!')
        
%% ********Get gt robot pose & desired robot pose & robot pose******* 
disp('figure 1...')
...
disp('Saved /euroc6/msf_core/odometry to variable!')


clearvars bagselect msgs
bagselect = select(bag, 'Topic', {'/euroc6/rci/controller/output/robot_goal_debug_pose'});
msgs = readMessages(bagselect);
disp('Loaded /euroc6/rci/controller/output/robot_goal_debug_pose from the bag file!')
...
disp('Saved /euroc6/rci/controller/output/robot_goal_debug_pose to variables!')

clearvars bagselect msgs
bagselect = select(bag, 'Topic', {'/euroc6/vrpn_client/estimated_odometry '});
msgs = readMessages(bagselect);
disp('Loaded /euroc6/vrpn_client/estimated_odometry from the bag file!')
...
for m = 1:length(msgs)   
    ...
    gt_robot_pos_world_x = [gt_robot_pos_world_x, msgs{m}.Pose.Pose.Position.X];
    gt_robot_pos_world_y = [gt_robot_pos_world_y, msgs{m}.Pose.Pose.Position.Y];
    gt_robot_pos_world_z = [gt_robot_pos_world_z, msgs{m}.Pose.Pose.Position.Z];
    gt_robot_or_world_x = [gt_robot_or_world_x, msgs{m}.Pose.Pose.Orientation.X];
    gt_robot_or_world_y = [gt_robot_or_world_y, msgs{m}.Pose.Pose.Orientation.Y];
    gt_robot_or_world_z = [gt_robot_or_world_z, msgs{m}.Pose.Pose.Orientation.Z];
    gt_robot_or_world_w = [gt_robot_or_world_w, msgs{m}.Pose.Pose.Orientation.W];
end
disp('aved /euroc6/vrpn_client/estimated_odometry to variable!')

clearvars bagselect msgs

%% ********Plot gt robot position & desired robot position & robot position******* 
f1=figure(1);
clf(f1);
x_robot = subplot(3,1,1);       % add first plot in 3 x 1 grid
plot(gt_robot_pos_world_time, gt_robot_pos_world_x, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
title('X coord of robot in world frame');
xlabel('time [s]'); ylabel('coordinate [m]');
legend('Ground Truth','Desired Position', 'From State Estimation','Location', 'northeastoutside')
...
plot(gt_robot_pos_world_time, gt_robot_pos_world_y, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
title('Y coord of robot in world frame');
xlabel('time [s]'); ylabel('coordinate [m]');
legend('Ground Truth','Desired Position', 'From State Estimation','Location', 'northeastoutside')
set(gca,'XMinorTick', 'on', 'YMinorTick', 'on','YGrid', 'on', 'YMinorGrid', 'on');
ylim_y=get(gca,'YLim');
...
plot(gt_robot_pos_world_time, gt_robot_pos_world_z, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
title('Z coord of robot in world frame');
xlabel('time [s]'); ylabel('coordinate [m]');
legend('Ground Truth','Desired Position', 'From State Estimation','Location', 'northeastoutside')
set(gca,'XMinorTick', 'on', 'YMinorTick', 'on','YGrid', 'on', 'YMinorGrid', 'on');
ylim_z=get(gca,'YLim');
...
clearvars bagselect msgs
bagselect = select(bag, 'Topic', {'/euroc6/rci/cavity_normal/output/cavity_pose'});
msgs = readMessages(bagselect);
disp('Loaded /euroc6/rci/cavity_normal/output/cavity_pose from the bag file!')
...
disp('Saved /euroc6/rci/cavity_normal/output/cavity_pose to variables!')

%%
gt_cavity_pos_world = [2.644,0.2097,0.6957];

gt_cavity_pos_robot_x = []; gt_cavity_pos_robot_y = []; gt_cavity_pos_robot_z = [];
for m = 1:length(gt_robot_pos_world_time)   
    robot_pos_world = [gt_robot_pos_world_x(m), gt_robot_pos_world_y(m), gt_robot_pos_world_z(m)];
    robot_or_world = [gt_robot_or_world_w(m), gt_robot_or_world_x(m), gt_robot_or_world_y(m), gt_robot_or_world_z(m)];
...
clf(f3);
x_robot = subplot(3,1,1);       % add first plot in 3 x 1 grid
plot(gt_robot_pos_world_time, gt_cavity_pos_robot_x, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
plot(no_meas_cavity_pos_robot_time, ones(1, length(no_meas_cavity_pos_robot_time))*1.4, 'Color','m', 'LineStyle','none','Marker','*');
...
y_robot = subplot(3,1,2);       % add second plot in 3 x 1 grid
plot(gt_robot_pos_world_time, gt_cavity_pos_robot_y, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
z_robot = subplot(3,1,3);       % add third plot in 3 x 1 grid
plot(gt_robot_pos_world_time, gt_cavity_pos_robot_z, 'LineWidth',1, 'Color','k', 'LineStyle','-');
hold on
...
plot(no_meas_cavity_pos_robot_time, ones(1, length(no_meas_cavity_pos_robot_time))*0.4, 'Color','m', 'LineStyle','none','Marker','*');
...
new_gt_cavity_pos_robot_x = [gt_robot_pos_world_time(start_idx:end)', gt_cavity_pos_robot_x(start_idx:end)'];
new_gt_cavity_pos_robot_y = [gt_robot_pos_world_time(start_idx:end)', gt_cavity_pos_robot_y(start_idx:end)'];
new_gt_cavity_pos_robot_z = [gt_robot_pos_world_time(start_idx:end)', gt_cavity_pos_robot_z(start_idx:end)'];
...
text(xlim_x(2)+2,0.5,str_desired_x, 'VerticalAlignment','bottom', 'HorizontalAlignment','left', 'Parent', x_robot);
...
text(xlim_y(2)+2,-0.4,str_desired_y, 'VerticalAlignment','bottom', 'HorizontalAlignment','left', 'Parent', y_robot);
...
text(xlim_z(2)+2,-0.4,str_desired_z, 'VerticalAlignment','bottom', 'HorizontalAlignment','left', 'Parent', z_robot); 
