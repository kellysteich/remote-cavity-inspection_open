...
void QCPLayoutElement::update(UpdatePhase phase)
{
  if (phase == upMargins)
  {
    if (mAutoMargins != QCP::msNone)
    {
      ...
      BOOST_FOREACH (QCP::MarginSide side, allMarginSides)
      {
...
{
  BOOST_FOREACH (QCPLayoutElement* el, elements(false))
  {
    if (!el->parentPlot())
...
QCPItemAnchor::~QCPItemAnchor()
{
  ...
  BOOST_FOREACH (QCPItemPosition *child, mChildrenX.toList())
  {
  ...
  BOOST_FOREACH (QCPItemPosition *child, mChildrenY.toList())
  {
...
{
  QList<QCPAbstractPlottable*> result;
  BOOST_FOREACH (QCPAbstractPlottable *plottable, mPlottables)
  {
...
{
  QList<QCPGraph*> result;
  BOOST_FOREACH (QCPGraph *graph, mGraphs)
  {
    if (graph->selected())
      result.append(graph);
  }
...
{
  QList<QCPAbstractItem*> result;
  BOOST_FOREACH (QCPAbstractItem *item, mItems)
  {
    if (item->selected())
      result.append(item);
  }
...
{
  BOOST_FOREACH (QCPLayer *layer, mLayers)
  {
    if (layer->name() == name)
      return layer;
  }
...
QList<QCPAxisRect*> QCustomPlot::axisRects() const
{
  ...
  {
    BOOST_FOREACH (QCPLayoutElement *element, elementStack.pop()->elements(false))
    {
      if (element)
      {
...
QCPLayoutElement *QCustomPlot::layoutElementAt(const QPointF &pos) const
{
  ...
  while (searchSubElements && currentElement)
  {
    ...
    BOOST_FOREACH (QCPLayoutElement *subElement, currentElement->elements(false))
    {
...
QList<QCPAxis*> QCustomPlot::selectedAxes() const
{
  ...
  BOOST_FOREACH (QCPAxisRect *rect, axisRects())
    allAxes << rect->axes();
  
  BOOST_FOREACH (QCPAxis *axis, allAxes)
  {
...
QList<QCPLegend*> QCustomPlot::selectedLegends() const
{
  ...
  while (!elementStack.isEmpty())
  {
    BOOST_FOREACH (QCPLayoutElement *subElement, elementStack.pop()->elements(false))
    {
      if (subElement)
      {
...
void QCustomPlot::deselectAll()
{
  BOOST_FOREACH (QCPLayer *layer, mLayers)
  {
    BOOST_FOREACH (QCPLayerable *layerable, layer->children())
...
void QCustomPlot::draw(QCPPainter *painter)
{
  ...
  BOOST_FOREACH (QCPLayer *layer, mLayers)
  {
    BOOST_FOREACH (QCPLayerable *child, layer->children())
    {
  ...
  BOOST_FOREACH (QCPLayoutElement* el, findChildren<QCPLayoutElement*>())
  {
    painter->setBrush(Qt::NoBrush);
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  if (mType != type)
  {
    ...
    BOOST_FOREACH (QCPAxis::AxisType atype, allAxisTypes)
    {
  ...
  mGradientImageInvalidated(true)
{
  ...
  BOOST_FOREACH (QCPAxis::AxisType type, allAxisTypes)
  {
...
{
  BOOST_FOREACH (QCPBars *bars, mBars) // since BOOST_FOREACH takes a copy, removing bars in the loop is okay
...
double QCPBarsGroup::keyPixelOffset(const QCPBars *bars, double keyCoord)
{
  ...
  BOOST_FOREACH (const QCPBars *b, mBars)
  {
    while (b->barBelow())
      b = b->barBelow();
